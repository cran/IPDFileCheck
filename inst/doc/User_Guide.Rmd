...
The function testFileExistenceReadability tests if the user provided file exists and is readable. Returns 0 for success and errors for failures. Here in the example a directory named "nodir" is generated and tested.
```{r }
...
####2. To check if the column exists
The function checkColumnExists tests if the column with  user specified column name exists in the data. For example in the above simulated data set 'rctdata', the column with column name 'sex' exists but 'gender' do not. Thus the function returns 0 when 'sex' is used while returns error when 'gender' is used.
```{r }
